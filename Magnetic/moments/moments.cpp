...
int main()
{
    ...
    for(i = 0; i < d+1; ++i)
    {
         outfile2 << std::scientific << pow(realtype(-1), realtype(i+2))*( boost::math::factorial <realtype> ( 2* (i + 2) - 3 )*(realtype(2))*(realtype(1)-pow(realtype(2), realtype( 2*(i+2) -1 ))) * boost::math::bernoulli_b2n <realtype> ( i + 2 ) ) / boost::math::factorial <realtype> ( 2 * ( i + 2 ) ) << std::endl;
    }
