...
int main()
{
    ...
    for ( i = 0; i < betas ; ++i )
    {
        ...
        fifthter(i) =  (I / sqrt( beta(i) ) ) * inner_prod( constants, term1 ); // the summation part of g(x), ie w/o the exponential

        outfile1 << std::scientific << ( Pi * sqrt( beta(i) )/ realtype(2) ) * real( exp( -I /( realtype(2) * sqrt(beta(i) ) ) ) * fifthter(i) )
                                               + ( sqrt(beta(i)) * log( beta(i) ) /realtype(2) ) * imag( exp( -I /( realtype(2)*sqrt(beta(i) ) ) ) * fifthter(i) ) << std::endl;
