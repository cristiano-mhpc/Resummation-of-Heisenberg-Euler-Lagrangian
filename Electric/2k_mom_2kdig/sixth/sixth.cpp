...
//ublas vectors
vector<realtype>  first_term(d+1), term1(d+1);
vector<realtype> first_fifthter(betas) ;
...
int main()
{
    ...
    outfile1.open("../results/SIXTH.txt",std::ios_base::out);
    ...
    for ( i = 0; i < betas ; ++i )
    {
        ...
        outfile1 << std::scientific << ( Pi/realtype(2) )*pow( beta(i), realtype(3)/realtype(2) )*(  ( exp( realtype(-1) / ( realtype(2)*sqrt(beta(i) ) ) ) * first_fifthter(i) )  ) << std::endl;
