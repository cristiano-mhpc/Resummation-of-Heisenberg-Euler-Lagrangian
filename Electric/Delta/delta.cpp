...
int main(int argc, char** argv)
{
   ...
   // outfile.precision(digits);
    ...
    for ( int i = 0; i < d+1; ++i )
    {
        moments(i) = ( pow(realtype(-1), realtype( i ) ) * ( pow(realtype(2), realtype( 2*(i+2) ) ) )
         * ( realtype(1) - pow(realtype(2), realtype(2*i + 3) ) ) * boost::math::bernoulli_b2n <realtype> ( i + 2 ) )
            / ( pow(realtype(2), realtype(2*i + 3)) * realtype(2*i + 4) * realtype( 2*i + 3 ) * realtype( 2*i + 2 ) );
    ...
    //outfile1.open("delta_50.txt",std::ios_base::out);
