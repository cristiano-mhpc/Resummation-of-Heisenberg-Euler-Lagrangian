...
int main()
{
    ...
    outfile2.open("moments.txt",std::ios_base::out);
    outfile2.precision(digits);

    for(i = 0; i < d+1; ++i)
    {
         outfile2 << std::scientific << ( pow(realtype(-1), realtype( i ) ) * ( pow(realtype(2), realtype( 2*(i+2) ) ) )
         * ( realtype(1) - pow(realtype(2), realtype(2*i + 3) ) ) * boost::math::bernoulli_b2n <realtype> ( i + 2 ) )
            / ( pow(realtype(2), realtype(2*i + 3)) * realtype(2*i + 4) * realtype( 2*i + 3 ) * realtype( 2*i + 2 ) ) << std::endl;
    }
